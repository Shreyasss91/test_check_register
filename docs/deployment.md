...
| Account-ID search says "No meter found" though the account is in Master (RR search works) | The lookup index was cached by a pre-v1.13.4 build, which keyed numeric accounts by their `4.26E+09` display instead of the real digits — and Master's unchanged row count kept it looking fresh. Fixed in v1.13.9 by a namespace bump (`mridx_v1_*` → `mridx_v2_*`): deploy Code.gs, and the next lookup rebuilds automatically. A duplicated Account ID in Master shows a separate "matches more than one meter" message — type the meter's RR Number instead |
