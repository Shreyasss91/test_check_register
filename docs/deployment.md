...
| Form card still shows `DOS: ########` or `Acct: 4.26E+09` after fixing Master | First check the form footer version matches the changelog — if it's older, the old version is still deployed (redeploy). If the version is current, the cell's stored value is literally the text `########` — replace it with the real date (the form shows `—` for such cells). Once Master col H is a real date, the card always renders `yyyy-MM-dd` regardless of column width |
| Account-ID search says "No meter found" though the account is in Master (RR search works) | Since v1.14.1 the form never trusts an index miss: it re-checks with a direct Master scan, serves the meter, and rebuilds the index on the spot (unknown numbers are remembered ~5 min so typo-retries stay fast). The historical cause was a pre-v1.13.4 cache keyed by `4.26E+09` displays — fixed by the `mridx_v2_*` namespace bump. Use menu *Meter Register > Meter index diagnostics…* to see the index's live state and test a number against it. A duplicated Account ID in Master shows a separate "matches more than one meter" message — type the meter's RR Number instead |
